...
func TestJoinGameRoundTrip(t *testing.T) {
	...
	decoded := _roundTrip(t, gocraft.StatePlay, gocraft.Clientbound, original)
...
func TestPlayKeepAliveDistinctIDsPerDirection(t *testing.T) {
	if (v765.PlayKeepAlive{}).ID() == (v765.PlayKeepAliveResponse{}).ID() {
		t.Fatal("clientbound and serverbound play keep-alive must have distinct ids")
	}

	received := &v765.PlayKeepAlive{KeepAliveID: 555}
	echoed := _roundTrip(t, gocraft.StatePlay, gocraft.Clientbound, received)
	...
	confirmed := _roundTrip(t, gocraft.StatePlay, gocraft.Serverbound, reply)
...
func TestConfirmTeleportRoundTrip(t *testing.T) {
	...
	decoded := _roundTrip(t, gocraft.StatePlay, gocraft.Serverbound, original)
