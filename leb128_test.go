...
func _boundaries[T varintDomain]() []T {
...
func _stdlibLeb128[T varintDomain](v T) []byte {
...
func TestAppendVarAgainstStdlib(t *testing.T) {
	for _, value := range _boundaries[int32]() {
		got := gocraft.AppendVar(nil, value)
		want := _stdlibLeb128(value)
	...
	for _, value := range _boundaries[int64]() {
		got := gocraft.AppendVar(nil, value)
		want := _stdlibLeb128(value)
